...
import (
	...
	"github.com/liuyong-go/dbreverse/util"
	"gorm.io/gorm"
...
func NewGenerator(conn_str string, path string, schema string) *Generator {
	db := DB(conn_str)
...
func (ge Generator) generateModel(table Table, fields []Field) {
	...
	content += "type " + util.CamelCase(table.Name) + " struct {\n"
	//生成字段
	var fieldName = ""
	for _, field := range fields {
		if field.Field == "id" {
			...
			fieldName = util.CamelCase(field.Field)
		}
	...
	content += "}"
	content += "\n"
	content += "func (*" + util.CamelCase(table.Name) + ") TableName() string {\n"
	content += "	return \"" + table.Name + "\"\n"
	content += "}"
	filename := ge.ModelsPath + util.CamelCase(table.Name) + ".go"
	var f *os.File
	var err error
	if checkFileIsExist(filename) {
		fmt.Println(util.CamelCase(table.Name) + " 已存在，需删除才能重新生成...")
		return
	} else {
	...
	if err != nil {
		...
		fmt.Println(util.CamelCase(table.Name) + " 已生成...")
	}
}
